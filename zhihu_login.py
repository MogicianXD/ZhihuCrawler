...
from zhihu_captcha.zhihu_captcha import ZhihuCaptcha

class ZhihuAccount(object):

    def __init__(self, captcha_model, username: str = None, password: str = None):
        ...
        self.session.cookies = cookiejar.LWPCookieJar(filename='cache/cookies.txt')
    ...
    def _get_captcha(self, lang: str):
        ...
        if show_captcha:
            ...
            img = Image.open('cache/captcha.png')
            if lang == 'cn':
                ...
                capt = self.captcha_model.recgImg(img)
                print(capt)
            # 这里必须先把参数 POST 验证码接口
            time.sleep(2)  # 这里如果不等几秒的话，知乎会报“提交过快，请稍后重试”，post失败
